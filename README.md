...
Each job keeps the raw location text and may also store a confirmed canonical location in `locationCanonical`, `locationCountryCode`, `locationCountry`, `locationRegion`, `locationCity`, `locationLatitude`, `locationLongitude`, `locationPrecision`, and `locationResolutionSource`. Existing CSVs remain compatible: the Viz page first checks these saved fields, then another saved job with the same normalized raw location, and then the bundled place catalog. Editing a job preserves its confirmed location while the raw text is unchanged and clears it when the raw location changes.
...
To deploy, enable GitHub Pages for the repository root. Include `index.html`, `styles.css`, `app.js`, `viz.css`, `viz.js`, `viz-geography.js`, `locations.js`, `research.css`, `research.js`, and the `img` directory. No build step or chart CDN is required.
...
The description panel reads locally saved job-description text and builds separate profiles for each role or industry instead of flattening unrelated jobs into one list. It can analyze requirements, responsibilities, or whole descriptions. Each profile separates tools and platforms, methods and domain knowledge, ways of working, and credentials; it also mines repeated multi-word phrases that fall outside the recognized skill library. Percentages show within-group description coverage, while a distinctiveness measure compares that coverage with the rest of the selected market. Selecting a signal displays short source excerpts and opens the matching jobs, and Copy profile creates a compact set of drafting notes.
...
- **Location analysis:** Remote, Location not recorded, and geographic applications are separate views; Remote and missing records never appear as geographic pins. Country tabs and their city or metro lists are generated from the selected application cohort. The map can size and label places by job count or average annualized pay, including hourly pay at 2,016 hours/year. Each place shows pay coverage, roles, outcomes, and its underlying jobs.
- **Location review:** Unrecognized raw locations appear in a review queue under the inferred country when possible. The user can map one to an existing saved place, create a city/metro/region manually, leave it reviewed but unmapped, or request OpenStreetMap Nominatim suggestions. A confirmed resolution updates matching jobs and is saved in `jobs.csv`, so the same raw location is reused in future. Suggestion search is never automatic: the raw location is sent to Nominatim only after the user clicks **Find suggestions**, and the user must choose a result before it is saved.
...
Chart selections open the underlying job list; selecting a job opens its existing edit form. Controls and SVG marks support keyboard activation. Charts are read-only; the location review controls modify only the explicit canonical location fields described above.

Run the calculation checks with `node --test tests/viz.test.cjs`.

The bundled country metadata and outlines in `viz-geography.js` are derived from [Natural Earth 1:110m data](https://github.com/nvkelso/natural-earth-vector), which is [public domain](https://www.naturalearthdata.com/about/terms-of-use/). Regenerate them with `node scripts/build-viz-maps.cjs` (Node 18+ and network access). The dashboard draws maps locally. Its only map-related network request is a user-triggered search against [OpenStreetMap Nominatim](https://nominatim.org/release-docs/develop/api/Search/), subject to the public service's [usage policy](https://operations.osmfoundation.org/policies/nominatim/); chosen results are cached in the job data.
